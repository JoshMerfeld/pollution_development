...
coefsdist <- readRDS("pollution_development/draft/tables/randomization_coefs_distfe.rds")
truevalue <- readRDS("pollution_development/draft/tables/randomization_true_value.rds")
...
# load rastlist
rastlist <- read_csv("data/counterfactual/rastlistclean.csv")
yield <- read_csv("data/counterfactual/yieldclean.csv")
dist <- read_sf("data/counterfactual/distclean.shp")
# merge yield into dist
dist <- dist %>% left_join(yield, by = c("dist_merge"))
# counterfactual
...
<!-- 
Jonathan's comments:
- Drop "in a developing country" from the title?
- Add a sense of magnitudes, specifically the effects of pollution on yields. Like "pollution has increased on avearge by X percent in India, and this has led to a Y percent decrease in yields"
  - This is important
- Build up labor productivity bit a little more
  - Jonathan thinks current writing makes it sound more like a technical point than a big deal (which it is)
- Look at pollution prior to season? Kind of a "placebo" test
- Maybe move labor and coal parts up in the introduction
- Add a citation on the difference between PM 2.5 and PM 10
  - Jonathan says he didn't understand the paragraph on PM 2.5 until we discussed it. Important to make sure the reader understands it.
-->
...
In 2021, energy generation from coal reached an all-time high,[^1] driven in part by rapid population growth in developing countries, including India. While natural gas has become a popular alternative in much of the world, the Indian government met much of the increased demand through the construction of coal power plants. From 1990 to 2010, India's population grew by more than 40 percent, leading to a large increase in demand for power.[^2] During those same two decades, power generation from all coal units of at least 30 megawatts in the country more than doubled, from 42.4 gigawatts to more than 100 gigawatts.[^3]
...
While electricity may lead to economic and productivity growth [@dinkelman2011effects; @kline2014local; @rud2012electricity; @van2017long], though electrification alone may not be a sufficient condition [@lee2020does], these improvements do not come without downsides, especially when it comes to power derived from coal. Emissions from the production and burning of coal -- pollutants like sulfur dioxide, nitrogen oxides, methane, and mercury, for example -- are also directly harmful to human health and the environment. Although the use of coal had been declining, recent events -- including Russia's invasion of Ukraine -- have led to an increase in the demand for coal.[^5]

[^5]: www.npr.org/2022/08/15/1117560560/a-rising-demand-for-coal-amidst-war-in-ukraine

Pollution more generally is also increasing in much of the world. Carbon dioxide equivalent per capita has almost doubled over the last 50 years in India, more than doubled in Indonesia, and almost tripled in China.^[https://edgar.jrc.ec.europa.eu] Much of this increase is due to the development process itself -- as countries develop, their economies transform, with more people moving out of agriculture and into manufacturing and services [@herrendorf2014growth]. Despite this development, agriculture remains a key sector in many developing countries, employing a large proportion of the poor [@dercon2009rural], and agriculture is a key sector for poverty reduction [@christiaensen2011evolving]. This raises an important question: do rising pollution levels in developing countries threaten agricultural productivity?
...
The results are consistent with evidence of effects of pollution on different forms of labor activity. For example, general levels of pollution decrease productivity of call center works in China [@chang2019effect], lead people to perform worse on cognitive functions [@ebenstein2016long; @wen2022lower], lower labor market earnings [@borgschulte2022air], and also lead to lower levels of farm labor productivity in California, driven by changes in ozone concentration [@graff2012impact]. Also in China, @he2019severe find small but significant negative effects of pollution on manufacturing productivity, while @chen2022effect show pollution leads to labor out migration. Relevant to my results, @hanna2015effect show that decreases in pollution lead to increases in labor allocation in Mexico.
...
rownames(plantresultstable) <- c("Population (log)", "", "Area (log)", "", "Ag. productivity", "", "Pollution (log PM 2.5)", "", "Has plant?", "",
                                "Observations")
kable(    
...
rownames(yield1reducedform) <- c("wind days (10s)", " ", 
...
kable(
  ...
  footnote(
           "Note: Standard errors are in parentheses and are clustered at the village level. The outcome in all regressions is estimated (log) agricultural yield, defined as tons per hectare. * p<0.10 ** p<0.05 *** p<0.01", 
           general_title = "",
           threeparttable = TRUE,
           footnote_as_chunk = TRUE
           ) %>%
...
kable(
  ...
  footnote(
           "Note: Standard errors are in parentheses and are clustered at the village level. The outcome in all regressions is estimated (log) agricultural yield (tons per hectare). * p<0.10 ** p<0.05 *** p<0.01", 
           general_title = "",
           threeparttable = TRUE,
           footnote_as_chunk = TRUE
           ) %>%
...
kable(
  ...
  footnote(
           "Note: Standard errors are in parentheses and are clustered at the village level. The outcome in all regressions is estimated (log) agricultural yield (tons per hectare). The rice dummy equals one in villages where rice is the most commonly harvested crop. * p<0.10 ** p<0.05 *** p<0.01", 
           general_title = "",
           threeparttable = TRUE,
           footnote_as_chunk = TRUE
           ) %>%
...
```{r yield8monthly, include = TRUE, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "Effects of pollution on productivity by month", fig.footer = "Coefficients are from a single regression, where the outcome is (log) agricultural yield and pollution in each of the five months of the season is instrumented with wind in each month. Standard errors are indicated by the bars and are clustered at the village level."}
...
Figure \@ref(fig:yield8monthly) presents the coefficients of the IV regression separately for each month of the agricultural season, with the very beginning of the season on the left side of the graph and the harvest season on the right side of the graph. Interestingly, the coefficient for the fourth month of the season is actually positive. It is hard to know exactly why this is the case, but it could be an interaction between the positive effects of coal pollution on agricultural productivity found by @sanders2022adaptation and the fact that very little labor is used just prior to harvest. However, the overall explanation is beyond the scope of the current paper.
...
# Overall effects of pollution on agricultural productivity
...
For this reason, I instead turn to data from ICRISAT,^[http://data.icrisat.org/dld] which has district-level total production data from different years. To track with the timing of the main data used above, I pull total agricultural production from 2001 and 2011 for three of the most common crops in India: rice, wheat, and maize.^[Maize might not seem like the first choice for India. However, maize is one of the most common crops grown in other developing countries -- especially in Sub-Saharan Africa -- so I use it here as a benchmark.] To simplify the calculation, I assume total area devoted to each crop is unchanged across the decade. I then merge this with the pollution data from @hammer2020global using a 2011 district-level census shapefile, which has identifiers that match the ICRISAT data. Figure \@ref(fig:yielddatamaps) in the appendix shows the distribution of total output for the three crops in 2001. The data covers much, but not all, of the country.

In the output data, I observe values for 2001 and 2011. I can calculate the overall change in log output across the two years, as well as the overall change in pollution levels. I use the most conservative point estimate *without* coal plants in Table \@ref(tab:yield7ivdiffindiff), which is $-0.223$. For each district, I calculate the counterfactual for each crop as follows:
...
for (col in 2:4){
  sumtable[1, col] <- commaNum(as.numeric(sumtable[1, col]))
  sumtable[2, col] <- commaNum(as.numeric(sumtable[2, col]))
  sumtable[3, col] <- commaNum(as.numeric(sumtable[3, col]))
}

kable(
      sumtable, caption = "Change in output and counterfactual, all districts",
      align = "lcccc", booktabs = TRUE, linesep = "", row.names = FALSE
      ) %>%
  footnote(
           "Note: Output is the total sum across all districts, in '000s of tons. The counterfactual is calculated as described in Equation 5.", 
           general_title = "",
           threeparttable = TRUE,
           footnote_as_chunk = TRUE
           ) %>%
...
```{r randomplot, echo = FALSE, message = FALSE, warning = FALSE, dev='png', fig.height = 4, fig.cap = "Randomization inference", fig.footer = list("Note: The figure presents the distribution of coefficients by randomly assigning wind direction and pollution exposure across villages within the same year. Sampling is done within year and district. The vertical line is the actual coefficient from column (4) of Table 3.")}
...
# plot
gg1 <- ggplot() + 
  geom_density(aes(x = coefsdist, fill = "within district"), alpha = 0.5, show.legend = FALSE) +
  scale_fill_manual(
    name = "randomization:",
    values = colors
  ) +
...
gg1 <- gg1 + 
  geom_vline(aes(xintercept = truevalue, color = "actual")) +
  scale_color_manual(
    name = "",
    values = colors,
    labels = labels
...
```{r yielddatamaps, include = TRUE, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "District-level yields in 2001", fig.footer = "Yield data comes from ICRISAT."}

g1 <- ggplot() +
        geom_sf(data = dist, color = NA) +
        geom_sf(data = dist %>% filter(is.na(log(riceproduction))==F), aes(fill = log(riceproduction)), color = NA) +
        scale_fill_viridis_c("log ('000 tons)", option = "viridis") +
        theme_minimal() +
        labs(title = "Rice")
g2 <- ggplot(dist) +
        geom_sf(data = dist, color = NA) +
        geom_sf(data = dist %>% filter(is.na(log(wheatproduction))==F), aes(fill = log(wheatproduction)), color = NA) +
        scale_fill_viridis_c("log ('000 tons)", option = "viridis") +
        theme_minimal() +
        labs(title = "Wheat")
g3 <- ggplot(dist) +
        geom_sf(data = dist, color = NA) +
        geom_sf(data = dist %>% filter(is.na(log(maizeproduction))==F), aes(fill = log(maizeproduction)), color = NA) +
        scale_fill_viridis_c("log ('000 tons)", option = "viridis") +
        theme_minimal() +
        labs(title = "Maize")
ggarrange(g1, g2, g3, legend = "bottom", common.legend = TRUE)
...
\begin{landscape}
```{r yield9counterfactual1, include = TRUE, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "Distribution of actual and counterfactual log changes in output, rice", dev = "pdf", fig.width = 8}

ggplot(yield) +
        geom_density(aes(x = rice_change, fill = "Actual"), color = "transparent", alpha = 0.5) +
        geom_density(aes(x = ricecounterfactual, fill = "Counterfactual"), color = "transparent", alpha = 0.5) +
        theme_minimal() +
        labs(x = "Change in log output", y = "Density", title = "Rice") +
        scale_fill_manual(name = "Output", values = c("Actual" = "orange", "Counterfactual" = "navy"))
...
\begin{landscape}
```{r yield9counterfactual2, include = TRUE, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "Distribution of actual and counterfactual log changes in output, wheat", dev = "pdf", fig.width = 8}


ggplot(yield) +
        geom_density(aes(x = wheat_change, fill = "Actual"), color = "transparent", alpha = 0.5) +
        geom_density(aes(x = wheatcounterfactual, fill = "Counterfactual"), color = "transparent", alpha = 0.5) +
        theme_minimal() +
        labs(x = "Change in log output", y = "Density", title = "Wheat") +
        scale_fill_manual(name = "Output", values = c("Actual" = "orange", "Counterfactual" = "navy"))
...
\begin{landscape}
```{r yield9counterfactual3, include = TRUE, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "Distribution of actual and counterfactual log changes in output, maize", dev = "pdf", fig.width = 8}

ggplot(yield) +
        geom_density(aes(x = maize_change, fill = "Actual"), color = "transparent", alpha = 0.5) +
        geom_density(aes(x = maizecounterfactual, fill = "Counterfactual"), color = "transparent", alpha = 0.5) +
        theme_minimal() +
        labs(x = "Change in log output", y = "Density", title = "Maize") +
        scale_fill_manual(name = "Output", values = c("Actual" = "orange", "Counterfactual" = "navy"))
