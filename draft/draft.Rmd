---
title: "Pollution, agricultural productivity, and development: Evidence from coal plants in India^[I thank Marc Bellemare and David Sungho Park for comments and suggestions that greatly improved the quality of the paper. The usual disclaimer applies. All scripts used in this paper are publicly available on my GitHub page: github.com/JoshMerfeld/pollution_development]"
author:
...
yieldtablehet1st <- readRDS("pollution_development/draft/tables/yieldtablehet1st.rds")
...
Using newly available estimates of India-wide agricultural (land) productivity, results show that the total number of days of being downwind from a coal power plant has a negative effect on overall agricultural productivity. Just a single additional day of being downwind from a coal power plant leads to a decrease in predicted yields of 0.3 percent. The within-village absolute deviation of wind exposure is around eight days, meaning that median changes in wind direction are responsible for fluctuations in agricultural productivity of around 2.4 percent, with changes of more than five percent from season to season being common. Using wind direction as an instrument for particulate matter, an increase of PM 2.5 by one microgram per cubic meter leads to an even larger decrease in agricultural productivity, at around 20 percent. This IV strategy is similar to that in @deryugina2019mortality, though they use general "high-pollution" wind directions while I concentrate only on wind directions from the location of a coal power plant. The IV estimates indicate that the effect of pollution is more negative than naïve OLS estimates indicate, which is consistent with the evidence on the construction of coal plants; higher levels of pollution are correlated with higher levels of agricultural productivity and economic growth, a relationship that the IV strategy adjusts downwards.
...
rownames(yieldtablehet)[c(1, 3)] <- c("PM 2.5", "PM time rain")
kable(
  ...
  add_header_above(c(" ", "exposure" = 2, "yield" = 2, " " = 1)) %>%
  kable_classic_2()
...
Although the results with district-year-season help mitigate this concern, Table \@ref(tab:yieldtablepostplant) in the appendix checks this possibility in a different way. Specifically, I restrict the sample to only village-years *after* the introduction of a coal plant. This drops any pre-plant-opening observations -- all of which are mechanically zeros for wind -- to prevent endogeneity due to the opening. The coefficients in Table \@ref(tab:yieldtablepostplant) show that the overall effects are essentially unchanged across any specifications. Comparing changes in coefficients can also help reduce any effects driven by other polluting industries located near coal plants; since the pre-opening exposure variable is zero, the coefficients in Table \@ref(tab:yieldtablepostplant) would likely increase in magnitude if the effects were driven by non-coal pollution.

The second robustness check includes leads and lags for wind to see if they predict current year productivity. I present these results in \@ref(tab:yieldtableleads). The first column is the reduced form effect and shows that only current wind predicts productivity; leads and lags have no impact. We see similar results in column two -- the IV results -- with a negative coefficient for current pollution, only. However, the standard errors are much larger than the main results -- around five times as large -- making it hard to come to any conclusions.
...
labortablerural <- rbind(mat1, labortablerural[c(1:2, 7:8),], mat2, labortableurban[-c(3:6),], labortableurban[c(3:6),])
...
labortableruralyoung <- rbind(mat1, labortableruralyoung[c(1:2, 7:8),], mat2, labortableruralold[-c(3:6),], labortableruralold[c(3:6),])

kable(
      labortableruralyoung, caption = "Wind direction and labor allocation by age (rural only)",
      align = "cccccc", booktabs = TRUE, linesep = ""
      ) %>%
  footnote(
           "Note: Standard errors are in parentheses and are clustered at the district level. Control variables include female, age, age squared, and (years of) education. The outcome in each regression is days worked in the previous seven days. * p<0.10 ** p<0.05 *** p<0.01", 
...
While speculative, one possible explanation for this pattern is a decrease in labor productivity. While the youngest may be able to readjust and increase their overall self-employment labor allocation, perhaps the oldest cannot due to, for example, health effects of pollution, which are well documented [@bentayeb2012adverse; @pope2000epidemiology].[^14] We see decreases in overall agricultural productivity, despite an increase in overall farm labor. We also see decreases in non-farm employment, indicating that individuals are reallocating labor from the non-farm sector to the farm sector.

[^14]: Much of the literature also suggests the youngest are at significant risk of mortality and morbidities. However, given my focus on labor allocation, I only discuss older adults here.
...
kable(
      yieldtablemonthly, caption = "Month-specific exposure",
      align = "cc", booktabs = TRUE, linesep = ""
      ) %>%
  footnote(
           "Note: Standard errors are in parentheses and are clustered at the village level. The outcome in all regressions is logged nightlights. * p<0.10 ** p<0.05 *** p<0.01", 
           general_title = "",
           threeparttable = TRUE,
           footnote_as_chunk = TRUE
           ) %>%
...
\newpage
```
```{r yieldtablehet1st, include = TRUE, echo = FALSE, message = FALSE, warning = FALSE}

# Table
colnames(yieldtablehet1st) <- c("PM", "PM", "PM", "PM", "PM", "PM*rain")
kable(
      yieldtablehet1st, caption = "Heterogeneity in the effects of pollution on productivity, first-stage results",
      align = "cccccc", booktabs = TRUE, linesep = ""
      ) %>%
  footnote(
           "Note: Standard errors are in parentheses and are clustered at the village level. The outcome in all regressions is estimated agricultural yield, defined as tons per hectare. The first two columns split the sample based on the median exposure (by village). The second two columns split the sample based on initial -- the first year available -- agricultural productivity. * p<0.10 ** p<0.05 *** p<0.01", 
  ...
  column_spec(c(2:7),width = "1.7cm") %>%
  row_spec(c(8), hline_after = TRUE) %>%
  row_spec(c(6), bold = TRUE) %>%
  add_header_above(c(" ", "(1)" = 1, "(2)" = 1, "(3)" = 1, "(4)" = 1, "(5)" = 2)) %>%
  kable_classic_2()
...
kable(
  ...
  footnote(
           "Note: Standard errors are in parentheses and are clustered at the village level. The sample is restricted to village-years after a coal plant was built near the village. The outcome in all regressions estimated agricultural yield (tons per hectare). * p<0.10 ** p<0.05 *** p<0.01", 
           general_title = "",
           threeparttable = TRUE,
           footnote_as_chunk = TRUE
           ) %>%
...
\newpage
```
```{r labortableoldstate, include = TRUE, echo = FALSE, message = FALSE, warning = FALSE}

# Table
colnames(labortableoldstate) <- c("all", "self", "wage", "farm", "non-farm")
kable(
      labortableoldstate, caption = "Wind direction and labor allocation, heterogeneity by age",
      align = "ccccc", booktabs = TRUE, linesep = ""
      ) %>%
  footnote(
           "Note: Standard errors are in parentheses and are clustered at the district level. Control variables include female, age, age squared, and (years of) education. The outcome in each regression is days worked in the previous seven days. * p<0.10 ** p<0.05 *** p<0.01", 
...
\newpage
```
```{r wagestable, include = TRUE, echo = FALSE, message = FALSE, warning = FALSE}

# Table
colnames(wagestable) <- c("farm", "non-farm", "all")
kable(
      wagestable, caption = "Wind direction and wages",
      align = "ccc", booktabs = TRUE, linesep = ""
      ) %>%
  footnote(
           "Note: Standard errors are in parentheses and are clustered at the district level. Control variables include female, age, age squared, and (years of) education. The outcome in each regression is the daily wage (log Rs per day). * p<0.10 ** p<0.05 *** p<0.01", 
           general_title = "",
           threeparttable = TRUE,
           footnote_as_chunk = TRUE
           ) %>%
